...
def evaluate(model, dataset, args):
    ...
    train_side, valid_side, test_side, item_side] = copy.deepcopy(dataset)
...
def recommend_items(model, dataset, args):
    ...
    # user = pd.concat([train, valid, test])
    ...
    # user_side = pd.concat([train_side, valid_side, test_side])
    ...
    for u in user:
        ...
        predictions = predictions[0]
        try:
            # rank = [predictions.argsort().argsort()[0].item(), predictions.argsort().argsort()[1].item(), predictions.argsort().argsort()[2].item(), ]
            rank = [predictions.argsort().argsort()[i].item() for i in range(5)]
            recommended_items.append(rank)
        except:
            recommended_items.append(rank)

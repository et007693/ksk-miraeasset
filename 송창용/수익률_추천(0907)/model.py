...
class SASRec(torch.nn.Module):
    ...
    def predict(self, user_ids, log_seqs, item_indices, log_seqs_side, item_indices_side): 
        ...
        # side information to tensor
        item_indices_side = torch.tensor(item_indices_side).view(1, len(item_indices_side)).to(self.dev)
        # 확률값 + (확률값 * 수익률 * 10)
        # if (logits<0) and (item_indices_side<0):
            # logits += logits * -item_indices_side * 100
        # else:
        logits += logits * item_indices_side * 100

...
def data_partition(fname):
    ...
    sidemap = {u: (i+1) for i, u in enumerate(set(f['side']))} 

    f['index'] = f['index'].map(umap)
    f['Code'] = f['Code'].map(smap)
    f['side'] = f['side'].map(sidemap)
...
def recommend_items(model, dataset, args):
    ...
    for u in user:
        ...
        predictions = -model.predict(*[np.array(l) for l in [[u], [seq], item_idx, [seq_side], item_side_idx]])
        predictions = predictions[0]
        # rank1 = [predictions.argsort().argsort()[i].item() for i in range(5)]
        smap2 = {v: k for k, v in smap.items()}
    ...
    recommended_data['item1'] = recommended_data['item1'].map(smap2)
    recommended_data['item2'] = recommended_data['item2'].map(smap2)
    recommended_data['item3'] = recommended_data['item3'].map(smap2)
    recommended_data['item4'] = recommended_data['item4'].map(smap2)
    recommended_data['item5'] = recommended_data['item5'].map(smap2)
    print(smap)
